...
function getTrapCountAndLastTimeForService(rows, service) {
    ...
    var lastTrapTime = dateFormat(new Date(lastRow.trapTime), "yyyy-mm-dd HH:MM:ss Z");
...
function statesToConsole(data, opts) {
    if (data.networkService) {
        ...
        var erpsServicesList = getErpsServices(data, opts);
        database.getTrapDetailsForTrapOid("1.3.6.1.4.1.18070.2.2.2.1.29.0.1", lastMinutesDate, statesToConsoleWithNotifications, erpsServicesList);
    }
}
