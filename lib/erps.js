...
function summaryToConsole(data, opts) {
    if (data.networkService) {
        ...
        erps.print();
        console.log(" ");
        var size = parseInt(erps.length(),10) -1; // remove header
        console.log("No of ERPS Services: (" + size + ")");
    }
}
...
function getTrapCountAndLastTimeForService(rows, service) {
    ...
    var lastTrapTime = "N/A";
    if(lastRow != null) {
        lastTrapTime = dateFormat(new Date(lastRow.trapTime), "yyyy-mm-dd HH:MM:ss Z");
    }
    
    return [count, lastTrapTime];
}

function statesToConsoleWithNotifications(rows, servicesAndOpts) {
    var erps = new columnizer();
    var erpsServicesList = servicesAndOpts[0];
    var opts = servicesAndOpts[1];
    
    if(opts.last == null) {
        erps.row("Service Name", "SVLAN", "Domain", "Current State", "State Changes", "Last State Change");
    } else {
    ...
    for (var j = 0; j < erpsServicesList.length; j++) {
        var service = erpsServicesList[j];
        var countAndLastTrapTime = getTrapCountAndLastTimeForService(rows, service);
        
        if(opts.last == null) {
            erps.row(service.name, service.id, service.managementDomainName, getCurrentRingState(service), countAndLastTrapTime[0], countAndLastTrapTime[1]);
        } else {
            erps.row(service.name, service.id, service.managementDomainName, getCurrentRingState(service), countAndLastTrapTime[0]);
        }
    }
    erps.print();
...
function statesToConsole(data, opts) {
    if (data.networkService) {
        ...
        var erpsServicesList = getErpsServices(data, opts);
        var servicesAndOpts = [erpsServicesList, opts]; 
        database.getTrapDetailsForTrapOid("1.3.6.1.4.1.18070.2.2.2.1.29.0.1", lastMinutesDate, statesToConsoleWithNotifications, servicesAndOpts);
    }
}
...
exports.summaryToConsole = summaryToConsole;