...
function detailToConsole(data, opts) {
    if (data.networkService) {
        for (var i = 0; i < data.networkService.length; i++) {
            if (data.networkService[i].id === opts.vlan && data.networkService[i].type === 'ERPS' && (opts.domain == null || data.networkService[i].managementDomainName === opts.domain)) {
                    ...
                    for (var k = 0; k < sne.switchList.length; k++) {
                        ...
                        for (var l = 0; l < ss.servicePortList.length; l++) {
                            ...
                            if (ss.erpsSettings && sp.erpsSettings) {
                                erps.row(sne.ipAddress, ss.index, portToString(sp),
                                ...
                                erps.row(sne.ipAddress, ss.index + ' (' + ss.serviceType + ')', portToString(sp),
                                    "",
                                    "",
                                    "",
                                    "");
