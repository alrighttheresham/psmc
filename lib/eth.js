...
function countsToConsole(data) {
    if (data.networkService) {
        ...
        ethTable.row("VLAN", "Name", "Type", "Domain");
        for (var i = 0; i < data.networkService.length; i++) {
            ethTable.row(data.networkService[i].id, helper.notNull(data.networkService[i].name), data.networkService[i].type, data.networkService[i].managementDomainName);
            if (data.networkService[i].type !== "GVRP") {
                minusGVRP++;
            }
            if (data.networkService[i].id > 4090) {
                above4090++;
            }
        }
        ethTable.print();
        console.log("");
        console.log("Total Ethernet Services (" + data.networkService.length + ")" + " excluding GVRP (" + minusGVRP + ")" + " excluding packetVX vlans (" + (minusGVRP - above4090) + ")");
    }
}
...
function portRefToSearchString(ref) {
    if(ref.type === "LAG") {
        return ref.switchReference.switchIndex + ":" + ref.type + ":" + ref.index;
    } else {
        return ref.switchReference.switchIndex + ":" + ref.shelfIndex + ":" + ref.slotIndex + ":" + ref.type + ":" + ref.index;
    }
}

function prettyUpCvlans(cvlans) {
    var list = [];
    for (var x = 0; x < cvlans.length; x++) {
        var cvlan = cvlans[x];
        var from = cvlan.fromVlanId;
        var to = cvlan.toVlanId;
...
function portToConsole(data, opts) {
    if (data.networkService) {
        var table = new columnizer();
        table.row("Ethernet Service", "Type", "Port Type", "CVLANs");
        
        var foundServicePort;
        
        for (var i = 0; i < data.networkService.length; i++) {
            var networkService = data.networkService[i];
            for (var j = 0; j < networkService.serviceNetworkElements.length; j++) {
                var serviceNetworkElement = networkService.serviceNetworkElements[j];
                if(serviceNetworkElement.ipAddress === opts.ne) {
                    for (var k = 0; k < serviceNetworkElement.switchList.length; k++) {
                        var serviceSwitch = serviceNetworkElement.switchList[k];
                        for (var l = 0; l < serviceSwitch.servicePortList.length; l++) {
                            var servicePort = serviceSwitch.servicePortList[l];
                            var searchPort = portRefToSearchString(servicePort.portReference);
                            if(searchPort === opts.port) {
                                foundServicePort = servicePort;
                                table.row((!serviceSwitch.serviceName ? "[BLANK]" : serviceSwitch.serviceName) + " (VLAN " + networkService.id + ")", 
                                    networkService.type, 
                                    servicePort.currentStatus === "DYNAMIC" ? "Dynamic" : "Provisioned", 
                                    prettyUpCvlans(servicePort.cvlanMappingList));
                            }
                        }
                    }
                }
        ...
        if(foundServicePort) {
            console.log("Port " + portRefToSearchString(foundServicePort.portReference) + " is a(n): " + foundServicePort.type);
            table.print();
        } else {
            console.log("Port not found in any services!");
        }
    }
}
...
exports.command = function (opts) {
    opts.command('eth')
        ...
        .option('counts', {
            abbr: 'z',
            flag: true,
            help: "get counts on the Ethernet Services"
        })
        .option('port', {
            abbr: 'x',
            metavar: 'PORT',
            help: "get list of Ethernet Services for port, format: 1:1:1:GIGE:1 or 1:LAG:1 or 1:1:1:XGIGE:1"
        })
        .option('detail', {
            abbr: 'd',
            metavar: 'VLAN',
...
exports.countsToConsole = countsToConsole;