...
var groupsToConsole = function (data) {
    if (data.group) {
        for (var i = 0; i < data.group.length; i++) {
            var devices = [];
            if (data.group[i].groupMemberList) {
...
var packsToConsole = function (data, opts) {
    if (data.shelfList) {
                    ...
                    if (data.shelfList[i].shelf[j].slot) {
                        for (var k = 0; k < data.shelfList[i].shelf[j].slot.length; k++) {
                            if (data.shelfList[i].shelf[j].slot[k].packShortName) {
                                if (opts.find.indexOf(data.shelfList[i].shelf[j].slot[k].packShortName.toUpperCase()) > -1 ) {
                                    //devices.add(data.shelfList[i].networkElement.address);
                                    var address = data.shelfList[i].networkElement.address;
                                    if (!map.get(address)) {
                                        map.set(address, 1);
                                    } else {
                                        map.set(address, map.get(address) + 1);
                                    }
                                }
                            }
                        }
        ...
        var noOfPacks = 0;
        map.forEach(function (value, key) {
            noOfPacks = noOfPacks + value;
        });
        console.log("Total number of Network Elements containing "+ opts.find + " pack (" + map.count() + ")");
        console.log("Total number of " + opts.find + " packs (" + noOfPacks + ")");
        var neList = "";
        map.forEach(function (value, key) {
            neList = neList + key + "(" + value + ") ";
        });
        console.log(neList);
    }
};
...
function walkTimeToConsole(data) {
    if (data.networkElement) {
        ...
        walkTimeTable.row("IP", "Sysname", "Type", "Version", "Uptime", "SNMP Walk Time");
        for (var i = 0; i < sortedList.length; i++) {
            walkTimeTable.row(sortedList[i].address, helper.notNull(sortedList[i].sysName), sortedList[i].type, helper.notNull(sortedList[i].version), sortedList[i].snmpWalkTime);
        }
        walkTimeTable.print();
    }
}
