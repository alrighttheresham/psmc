                ...
                if (shelfData.shelfList[i].ethernetLayer.switch[j].index ===
                    ...
                    for (var k = 0; k < shelfData.shelfList[i].ethernetLayer.switch[j].portList.length; k++) {
                        if (shelfData.shelfList[i].ethernetLayer.switch[j].portList[k].shelfIndex ===
                            ...
                            data.index && shelfData.shelfList[i].ethernetLayer.switch[j].portList[k].type === data.type) {
...
var callback = function (err, opts, shelfData) {
    if (err) {
        console.log(err);
    } else {
        helper.requestWithEncoding(opts, "NetworkServices", function (err, opts, data) {
            if (err) {
                console.log(err);
            } else {
                if (opts.filter) {
