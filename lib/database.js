...
exports.getTrapDetailsForTrapOid = function(trapOid, afterDate, callback, data) {
    var afterDateQuery = '';
    if(afterDate !== '') {
        afterDateQuery = "AND trapTime > '" + afterDate + "'"; 
    }
    
    var query = "SELECT deviceAddress, oid, value, trapTime FROM Notifications, NotificationValues WHERE id = notification_id AND trapOid = '" + trapOid + "' " + afterDateQuery + "ORDER BY trapTime;";
...
exports.getAlarmsForElement = function(element, callback) {

    var query = "SELECT * FROM AlarmsHistory WHERE deviceAddress = '" + element + "' " + " ORDER BY timeRaised DESC;";
